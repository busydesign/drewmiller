...
import { replaceImportedOpenHomes } from "@/lib/listing-import/save-open-homes";
import { slugify } from "@/lib/slugify";
import { TEAM_MEMBER_IDS } from "@/lib/team";
...
function formatAddress(
  address?: ApiListing["address"] & { unitNumber?: string }
) {
  if (!address) return "";
...
async function fetchMemberListings(
  ...
  return (json.data || [])
    ...
    .filter((v) => (v.agents || []).some((a) => a.memberId === memberId));
}

export type TeamListingSyncResult = {
  created: number;
  updated: number;
  total: number;
};

export async function syncTeamListings(
  log: (message: string) => void = console.log
): Promise<TeamListingSyncResult> {
  ...
  for (const memberId of TEAM_MEMBER_IDS) {
    ...
    log(`member ${memberId}: ${current.length} current, ${sold.length} sold`);
  ...
  for (const listing of listings.values()) {
    ...
    const listedAt = listing.creationTime
      ...
      : null;
    const openHomes = isSold
      ? []
      : parseRayWhiteInspections(listing.inspections);
    const auction = isSold ? null : parseRayWhiteAuction(listing.auction);
    ...
    const data = {
      ...
      auctionAt: auction?.auctionAt ?? null,
    ...
    if (isSold) {
      await prisma.listingOpenHome.deleteMany({ where: { listingId: saved.id } });
    } else {
      await replaceImportedOpenHomes(saved.id, openHomes);
    }

    if (existing) {
      updated++;
      log(`↻ ${address} · ${leadAgent?.name || "no agent"} (${status})`);
    } else {
      created++;
      log(`+ ${address} · ${leadAgent?.name || "no agent"} (${status})`);
    }
  ...
  return { created, updated, total: listings.size };
}
